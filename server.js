...
app.get("/", (req, res) => {
  res.type("text/plain").send("Premium proxy is running");
});
...
app.get("/api/sub/:key", async (req, res) => {
  ...
  if (isBrowser) {
    const username = decodeNameFromKey(key) || "Premium User";
  ...
  try {
    ...
    res.setHeader("profile-title", "Premium");
...
function openV2RayNG(){ location.href = "v2rayng://install-sub?url=" + encodeURIComponent(subUrl) + "&name=" + encodeURIComponent("Premium"); }
...
app.listen(port, "0.0.0.0", () => {
  console.log("Premium running on port " + port);
});
