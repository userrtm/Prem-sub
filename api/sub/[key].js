export default async function handler(req, res) {
  ...
  const subUrl = `https://premiumusertm.com/api/sub/${encodeURIComponent(key)}`;
  ...
  if (isBrowser) {
    const username = decodeNameFromKey(key) || "Premium UserrTM";
...
.card:before{
  content:"VPN FAST SECURE PREMIUM USERRTM VPN FAST SECURE PREMIUM USERRTM";
  position:absolute;
  top:40%;
  left:-40%;
  width:180%;
...
<div class="bgText">
  ...
  <span>PREMIUM VPN • PREMIUM USERRTM</span>
...
function openHiddify(){ location.href = "hiddify://import/" + subUrl + "#Premium"; }
function openV2RayTun(){ location.href = "v2raytun://import/" + subUrl; }
function openV2RayNG(){ location.href = "v2rayng://install-sub?url=" + encodeURIComponent(subUrl) + "&name=" + encodeURIComponent("Premium UserrTM"); }
